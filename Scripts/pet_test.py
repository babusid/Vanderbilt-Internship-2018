	...
	def data_in(self):
		...
		while not rospy.is_shutdown():
			...
			if self.primary_int.touch_body:
				...
				try:
					self.value = (self.primary_int.touch_body[0] + 2.0*self.primary_int.touch_body[1] + 3.0*self.primary_int.touch_body[2] + 4.0*self.primary_int.touch_body[3] )/(numpy.sum(self.primary_int.touch_body))
